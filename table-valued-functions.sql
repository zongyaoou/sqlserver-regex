...
CREATE FUNCTION dbo.RegexMatches (@input nvarchar(max), @pattern nvarchar(max),@option nvarchar(100) = 'None')
RETURNS TABLE (Position int, Match NVARCHAR(max))  
EXTERNAL NAME [RegexAssembly].[UDF].[Matches] 
go

CREATE FUNCTION dbo.RegexSplit (@input nvarchar(max), @pattern nvarchar(max),@option nvarchar(100) = 'None')
RETURNS TABLE (Position int, Match NVARCHAR(max))
